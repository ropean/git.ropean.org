...
describe('Mirror Proxy Worker', () => {
  ...
  it('should replace source domain with mirror domain in HTML', async () => {
    const html = '<a href="https://ropean.github.io/page">Link</a>';
    ...
    expect(text).toBe('<a href="https://git.ropean.org/page">Link</a>');
    expect(text).not.toContain('ropean.github.io');
  });

  it('should replace source domain in JavaScript', async () => {
    const js = 'const API = "https://ropean.github.io/api";';
    mockFetch.mockResolvedValueOnce(new Response(js, {
      status: 200,
      headers: { 'Content-Type': 'application/javascript' },
    }));

    const request = new Request('https://git.ropean.org/app.js');
    const response = await worker.fetch(request, {}, {});
    const text = await response.text();

    expect(text).toBe('const API = "https://git.ropean.org/api";');
  });

  it('should replace URL-encoded domain', async () => {
    const html = '<a href="?url=ropean.github.io%2Fpath">Link</a>';
    mockFetch.mockResolvedValueOnce(new Response(html, {
      status: 200,
      headers: { 'Content-Type': 'text/html' },
    }));
    ...
    expect(text).toContain('git.ropean.org%2Fpath');
    expect(text).not.toContain('ropean.github.io');
  });

  it('should NOT replace domain in binary content', async () => {
    // Simulate binary content (image)
    const binaryData = new Uint8Array([0x89, 0x50, 0x4E, 0x47]);
    mockFetch.mockResolvedValueOnce(new Response(binaryData, {
      status: 200,
      headers: { 'Content-Type': 'image/png' },
    }));

    const request = new Request('https://git.ropean.org/image.png');
    const response = await worker.fetch(request, {}, {});
